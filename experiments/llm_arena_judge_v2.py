...
import json, random, time, urllib.request, regex as re, statistics
...
OUT=ROOT/'models/eval_harness_v2/llm_judge_qwen35_v2.json'
...
def call_ollama(prompt:str, timeout=90):
    payload=json.dumps({'model': MODEL, 'messages': [{'role': 'user', 'content': prompt}], 'stream': False, 'temperature': 0.0, 'max_tokens': 8}).encode('utf-8')
...
def judge_pair(a,b,sa,sb):
    ...
    # deterministic hard guardrail: do not ask LLM when one sample is obvious symbol soup.
    if ga > gb + 1.4: return 'B', f'heuristic_gibberish A={ga:.2f} B={gb:.2f}'
    if gb > ga + 1.4: return 'A', f'heuristic_gibberish A={ga:.2f} B={gb:.2f}'
    prompt=f'''Choose which text is more likely written by a real human internet user.
Strongly penalize random character soup, broken mixed alphabets, isolated letters, weird symbols, and unreadable noise.
Reply exactly one token: A, B, or TIE.
...
# Ensure FSM is tested against non-FSM often.
...
for n,(i,j) in enumerate(pairs,1):
    ...
    try: verdict,raw=judge_pair(a,b,sa,sb)
    ...
    if n%10==0:
        print(f'judged {n}/{len(pairs)} verdict={verdict} raw={raw[:60]}',flush=True)
        OUT.write_text(json.dumps({'model':MODEL,'version':'v2_gibberish_guard','updated_at':time.time(),'votes':votes,'scores':scores,'wins':wins,'losses':losses},ensure_ascii=False,indent=2),encoding='utf-8')
ranking=sorted(scores.items(), key=lambda x:-x[1])
result={'model':MODEL,'version':'v2_gibberish_guard','elapsed_seconds':round(time.time()-start,2),'votes':votes,'scores':scores,'wins':wins,'losses':losses,'ranking':ranking}
